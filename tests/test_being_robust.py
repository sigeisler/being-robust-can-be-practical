...
class TestBeingRobust():
    ...
    def test_output_shape(self):
        ...
        br = BeingRobust(random_state=42, keep_filtered=True, assume_centered=True, debug=True).fit(X)
    ...
    def test_naive_gaussian(self):
        ...
        br = BeingRobust(random_state=42, debug=True).fit(X)
        np.allclose(br.location_, np.zeros(n_features), rtol=1e-3, atol=1e-5)

        # Full SVD
        br = BeingRobust(random_state=42, use_randomized_svd=False, debug=True).fit(X)
        np.allclose(br.location_, np.zeros(n_features), rtol=1e-3, atol=1e-5)
